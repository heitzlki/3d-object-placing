...
def read_stl(filename):
    ...
    v3 = data["Vertex3"]
    points = np.hstack(
        ((v1[:, np.newaxis, :]), (v2[:, np.newaxis, :]), (v3[:, np.newaxis, :]))
    )
    return points
...
def calc_volume(points):
    ...
    volume = np.asarray(
        [
            np.cross(v[i, 0, :], v[i, 1, :]).dot(v[i, 2, :])
            for i in range(points.shape[0])
        ]
    )
    return np.abs(volume.sum() / 6.0) / (10.0**3.0)
...
def iter_calc_volume(filename):
    ...
    return np.abs(volume / 6.0) / (10.0**3.0)
...
def test(filename):
    ...
    print(bb1, bb2)
    assert (
        np.isclose(bb1[0], bb2[0])
        and np.isclose(bb1[1], bb2[1])
        and np.isclose(bb1[2], bb2[2])
    )
